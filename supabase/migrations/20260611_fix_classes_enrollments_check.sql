-- Fix migration: add current_students (if missing) and only populate/create triggers
-- Safe to run even if class_enrollments does not exist yet.
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'Table public.class_enrollments does not exist: skipping population and trigger creation.';
  END IF;
END$$;
