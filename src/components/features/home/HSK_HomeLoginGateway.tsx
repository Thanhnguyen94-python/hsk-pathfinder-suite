...
const cards: {
  ...
  to: "/student" | "/teacher" | "/logistics" | "/care" | "/admin";
}[] = [
  ...
  { role: "CSKH", title: "CSKH", desc: "Quản lý tuyển dụng và chăm sóc KH.", icon: Users, tone: "success", to: "/care" },
    ...
    <section id="login" className="bg-background py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 overflow-x-auto">
          <div className="flex gap-4 min-w-max lg:grid lg:grid-cols-5 lg:min-w-full">
            {cards.map((c) => {
              const Icon = c.icon;
              return (
                <Link
                  key={c.role}
                  to={c.to}
                  className="group relative flex flex-col rounded-2xl border border-border bg-card p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:border-foreground/20 hover:shadow-lg w-72 lg:w-auto flex-shrink-0 lg:flex-shrink"
                >
                  <div
